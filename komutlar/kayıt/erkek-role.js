...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.permissions.has('ADMINISTRATOR')) return message.channel.send(nn.setImage('https://media.giphy.com/media/Y41ynggo39awUmeg70/giphy.gif').setTitle(`Bir Hata Oldu!`).setThumbnail(message.author.avatarURL({dynamic: true})).setDescription(`**•** \`${client.ayarlar.prefix}erkek-rol-ayarla\` **kullanmak için,** \`Yönetici\` **yetkisine sahip olman gerekiyor.**`)).then(a => a.delete({timeout: 10000}));
...
message.channel.send(nn.setTitle(`İşte bu kadar!`).setDescription(`Kayıt İçin Kullanılacak Erkek Rolünü: ${role} Rol Olarak Seçtiniz.`)).then(a => a.delete({timeout: 10000}));
